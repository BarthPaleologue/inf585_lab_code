...
vec3 particle_structure::evaluate_position(float absolute_time) const {
    ...
    float impact_time = 2 * v0[2] / 9.81f;
    vec3 impact_position = 0.5f * g * impact_time * impact_time + v0 * impact_time + p0;
    if (absolute_time - t0 > impact_time) {

        //p[2] = 0;
        p = 0.5f * g * (t - impact_time) * (t - impact_time) + v0 * (t - impact_time) + impact_position;
    }
    // ... to adapt
    // **************************************************************** //
