...
void divergence_free(cgp::grid_2D<cgp::vec2> &new_velocity, cgp::grid_2D<cgp::vec2> const &velocity,
                     cgp::grid_2D<float> &divergence, cgp::grid_2D<float> &gradient_field);

template<typename T>
void diffuse(cgp::grid_2D<T> &new_field, cgp::grid_2D<T> const &field_reference, float mu, float dt,
             boundary_condition boundary);

template<typename T>
void advect(cgp::grid_2D<T> &new_value, cgp::grid_2D<T> const &value_reference, cgp::grid_2D<cgp::vec2> const &velocity,
            float dt);
...
void diffuse(cgp::grid_2D<T> &new_field, cgp::grid_2D<T> const &field_reference, float mu, float dt,
    ...
    int const N = int(new_field.dimension.x);

    for (int k = 0; k < 10; ++k) {
        for (int x = 1; x < N - 1; ++x) {
            for (int y = 1; y < N - 1; ++y) {
                new_field(x, y) = (field_reference(x, y) + mu * dt * (new_field(x - 1, y) + new_field(x + 1, y) +
                                                                    new_field(x, y - 1) + new_field(x, y + 1))) /
                                  (1.0f + 4.0f * mu * dt);
            }
        }
        if (boundary == copy) {
