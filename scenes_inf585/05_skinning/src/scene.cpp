...
compute_skinning_weights_generic(cgp::mesh const &m, skeleton_structure const &skeleton, float power_factor) {
...
numarray<numarray<float> >
compute_skinning_weights_cylinder(cgp::mesh const &m, skeleton_structure const &skeleton, float power_factor) {
    int N_vertex = m.position.size();
...
void scene_structure::set_skinning_weights() {
    if (model_type == cylinder || model_type == bar) {
        model.rigged_mesh.skinning_weight = compute_skinning_weights_cylinder(model.rigged_mesh.mesh_bind_pose,
                                                                              model.skeleton,
                                                                              gui.power_factor_skinning_weight);
    }
    if (model_type == spot) {
        model.rigged_mesh.skinning_weight = compute_skinning_weights_generic(model.rigged_mesh.mesh_bind_pose,
                                                                             model.skeleton,
                                                                             gui.power_factor_skinning_weight);
    }
}
...
void scene_structure::display_gui() {
    ...
    ImGui::Indent();
    ImGui::Checkbox("Frames", &sk_drawable.display_joint_frame);
    ImGui::SameLine();
    ImGui::Checkbox("Spheres", &sk_drawable.display_joint_sphere);
    ImGui::SameLine();
    ...
    ImGui::Text("Model: ");
    ImGui::SameLine();
    bool type = ImGui::RadioButton("Cylinder", (int *) &model_type, cylinder);
    ImGui::SameLine();
    type |= ImGui::RadioButton("Bar", (int *) &model_type, bar);
    ImGui::SameLine();
    ...
    ImGui::Text("Motion: ");
    ImGui::SameLine();
    ImGui::Checkbox("Bending", &gui.is_bending_motion);
    ImGui::SameLine();
    ImGui::Checkbox("Twisting", &gui.is_twisting_motion);


    ImGui::SliderFloat("Time scale", &timer.scale, 0.0f, 2.0f);
    bool change_power = ImGui::SliderFloat("Power factor skinning weight", &gui.power_factor_skinning_weight, 0.5f,
                                           10.0f, "%.3f", 2);
    if (change_power) {
        set_skinning_weights();
    }
...
animated_model_structure create_bar() {
    ...
    float const a = 0.1f;
    model.rigged_mesh.mesh_bind_pose = mesh_primitive_cubic_grid({0, -a, -a}, {1, -a, -a}, {1, a, -a}, {0, a, -a},
                                                                 {0, -a, a}, {1, -a, a}, {1, a, a}, {0, a, a}, 35, 10,
                                                                 10);
